...
python3 "${CLAUDE_SKILL_DIR}/scripts/build_hwpx.py" \
  ...
  --title "문서 제목" \
  --output output/hwpx/{name}.hwpx

# 3. 네임스페이스 후처리 (필수!)
python3 "${CLAUDE_SKILL_DIR}/scripts/fix_namespaces.py" output/hwpx/{name}.hwpx

# 4. 검증
python3 "${CLAUDE_SKILL_DIR}/scripts/validate.py" output/hwpx/{name}.hwpx
```
...
OUTPUT = Path("output/hwpx/{name}.hwpx")
...
python3 "${CLAUDE_SKILL_DIR}/scripts/build_hwpx.py" \
  --header /tmp/ref_header.xml --section /tmp/new_section.xml --output output/hwpx/{name}.hwpx
python3 "${CLAUDE_SKILL_DIR}/scripts/fix_namespaces.py" output/hwpx/{name}.hwpx
```
...
```python
import subprocess
subprocess.run(["python3", f"{SKILL_DIR}/scripts/fix_namespaces.py", "output/hwpx/{name}.hwpx"], check=True)
```
